...
export abstract class ComputeBaseService {
  ...
  constructor(configService: ConfigService, configToken) {
    ...
    const { user, password, host, port } = configService.get(
      'db.couchDB.connect',
    );
    this.user = user;
    ...
    const { modules, jobProfile, sysSettingsKey, script } = configService.get(
      `${this.configToken}`,
    );
  ...
  generateBaseJobOpt(
    data: AllGenomesInput,
  ): jobManagerClient.jobOptProxyClient {
    const jobOpt = {
      exportVar: {
        rfg: this.rfg,
        gi: data.gi.join('&'),
