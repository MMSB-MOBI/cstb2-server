...
import {
  SpecificGeneInput,
  SpecificGeneResults,
} from '../computeSpecific/dto/computeSpecific.dto';
...
export class ComputeSpecificService extends ComputeBaseService {
  ...
  async specificGeneCompare(
    data: SpecificGeneInput,
  ) /* : Promise<SpecificGeneResults> */ {
    const jobOpt: jobManagerClient.jobOptProxyClient = super.generateBaseJobOpt(
      data,
    );
